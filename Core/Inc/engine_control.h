 ...
 * Typically configured with ID 0x512 (or similar) to drive the inverter directly.
 */
extern CAN_TxHeaderTypeDef TxHeaderTH;
...
/**
 * @brief Data buffer for Throttle/Torque payload (8 bytes).
 * Used to store the calculated torque value before transmission.
 */
 ...
 * @details Transmits `0x02` on ID 0x000.
...
/**
 * @brief  Sends the " "Pre-Operational" NMT command to the inverter.
 * @details Transmits 0x80 depending on protocol on ID 0x000.
